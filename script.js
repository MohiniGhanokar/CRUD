...
regForm.onsubmit=(e)=>{
    ...
    if(checkEmail == undefined)
    {
        ...
        profile:url&&url!==""?url:"Flowerimage.jpg"//for default image set
        });
    
   
     localStorage.setItem("allRegData",JSON.stringify(allRegData));
     swal("Data Inserted","Successfully !","success") 
     //for close the add new window after data inserted successfully
     closeBtn.click();
     regForm.reset('');  
     getRegData();
    }
      ...
      let finalData = dataStr.replace(/"/g,"'");
       ...
       regList.innerHTML+=`
                    <tr>
                        <td>${index+1}</td>
                        <td>
                           <img src="${data.profile||'Flowerimage.jpg'}" width="30" alt=""/>
                        </td>
                        <td>${data.name}</td>
                        <td>${data.email}</td>
                        <td>${data.mobile}</td>
    ...
    for(let btn of allDelBtn)
        {
           btn.onclick=async ()=>{
            ...
            localStorage.setItem("allRegData",JSON.stringify(allRegData))
            getRegData();  //for live delete after click
            }
           }
        }
        ...
        profile:url&&url!==""?url:"Flowerimage.jpg"//for default image set
          ...
          swal("Data Updated","Successfully !","success") 
            //for close the add new window after data updated successfully
              closeBtn.click();
              regForm.reset(''); 
              getRegData();
